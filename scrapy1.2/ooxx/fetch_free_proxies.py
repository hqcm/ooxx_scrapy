...
def get_html(url):
    headers={'User-Agent':'Mozilla/5.0 (Windows NT 6.1; WOW64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/55.0.2883.87 Safari/537.36','connection': 'close',}
...
def check_proxies(proxy):
    headers={'User-Agent':'Mozilla/5.0 (Windows NT 6.1; WOW64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/55.0.2883.87 Safari/537.36','connection': 'close',}
    crawl_url='http://jandan.net/ooxx'
    try:
        ...
        if r.status_code == 200 and r.url==crawl_url:
            page_number=re.findall(r'<span class="current-comment-page">\[(\d\d)\]</span>', r.body.decode())[0]
            print (page_number)
            logger.info ('valid %s' % proxy) 
            return True
        else:
            logger.info ('invalid %s' % proxy) 
            return False
    except:
