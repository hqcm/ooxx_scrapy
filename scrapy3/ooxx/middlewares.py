...
sys.path.append(r'C:\Users\Administrator\Desktop\scrapy_煎蛋网\test3\ooxx')
...
class HttpProxyMiddleware(object):
        ...
        def get_proxy(self,request):
            if len(self.proxies)<self.proxies_threshold:
        ...
        def fetch_new_proxies(self):
            ...
            if  self.proxies_threshold==1:
                logger.debug('代理列表中只剩下一个代理，暂停5分钟后再次抓取代理')
                time.sleep(300)
            elif len(self.proxies)<self.proxies_threshold:
                self.proxies_threshold-=1
